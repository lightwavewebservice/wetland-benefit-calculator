import React, { useCallback, useMemo, useState, createContext, useMemo as useReactMemo } from 'react';
import { ThemeProvider, CssBaseline, Tooltip, Typography, Collapse, Paper, Divider, Box, IconButton, useMediaQuery, useTheme } from '@mui/material';
  ...
  return (
    <Paper 
      ...
      sx={{
        ...
        '& .text-slate-700': {
          color: isDark ? 'rgba(255, 255, 255, 0.87)' : '#334155'
        },
        '& .text-slate-600': {
      ...
      <Collapse in={expanded}>
        <div className="mt-4 space-y-6">
          ...
          <div>
            ...
            <div className="bg-slate-50 p-4 rounded-md" style={{ backgroundColor: isDark ? 'rgba(255, 255, 255, 0.05)' : '#f8fafc' }}>
      ...
      <input
        ...
        className={`w-full rounded border px-3 py-2 text-sm focus:outline-none focus:ring-1 ${
                      theme.palette.mode === 'dark'
                        ...
                        : 'border-slate-300 bg-white text-slate-900 focus:border-primary focus:ring-primary'
                    }`}
        aria-label={label}
      />
  ...
  return (
    <ColorModeContext.Provider value={colorMode}>
      <ThemeProvider theme={theme}>
        ...
        <div className={`min-h-screen ${theme.palette.mode === 'dark' ? 'bg-slate-900 text-slate-50' : 'bg-slate-50 text-slate-900'}`}>
          <header className={`border-b ${theme.palette.mode === 'dark' ? 'border-slate-700 bg-slate-800' : 'border-slate-200 bg-white'}`}>
            <div className="mx-auto flex max-w-7xl items-center justify-between p-4">
              <div className="flex items-center space-x-4">
                <IconButton sx={{ ml: 1 }} onClick={colorMode.toggleColorMode} color="inherit">
                  {theme.palette.mode === 'dark' ? <Brightness7Icon /> : <Brightness4Icon />}
                </IconButton>
                <span className="text-sm font-medium">{theme.palette.mode === 'dark' ? 'Light' : 'Dark'} Mode</span>
              </div>
            </div>
          </header>
          <main className={`mx-auto flex max-w-7xl flex-col gap-4 px-4 py-6 lg:flex-row ${theme.palette.mode === 'dark' ? 'bg-slate-900' : 'bg-white'}`}>
            ...
            <aside className="flex w-full flex-col gap-4 lg:w-1/3">
              <div className={`space-y-4 rounded-lg border p-4 shadow-sm ${
                ...
                <div className="block space-y-1">
                  <label htmlFor="dem-export-url" className={`text-sm font-medium ${
                    theme.palette.mode === 'dark' ? 'text-white' : 'text-slate-700'
                  }`}>DEM Export URL</label>
                  <textarea
                    id="dem-export-url"
                    name="dem-export-url"
                    value={demUrl}
                  ...
                  />
                  <p id="dem-export-url-help" className={`text-xs ${
                    theme.palette.mode === 'dark' ? 'text-slate-300' : 'text-slate-500'
                  }`}>
                ...
                <div className="block space-y-1">
                  <label htmlFor="wetland-name" className={`text-sm font-medium ${
                    theme.palette.mode === 'dark' ? 'text-white' : 'text-slate-700'
                  }`}>Wetland Name</label>
                  <input
                    id="wetland-name"
              ...
              {result ? (
                ...
                <div className={`rounded-lg border border-dashed p-4 text-sm shadow-sm ${theme.palette.mode === 'dark' ? 'border-slate-600 bg-slate-800 text-slate-300' : 'border-slate-300 bg-white text-slate-500'}`}>
